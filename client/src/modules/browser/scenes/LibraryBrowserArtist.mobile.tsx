...
import { generatePath, NavLink, useNavigate, useParams } from 'react-router'
...
import { getArtist } from 'modules/browser/store'

export default function LibraryBrowserArtist() {
  const { artistId } = useParams()
  const navigate = useNavigate()

  const paneRef = useRef<HTMLDivElement>(null)
  const artist = useAppSelector((state) => getArtist(state, artistId!))
  ...
  return (
    <Container>
      <Header>
        <BackButton to={ROUTES.libraryBrowserArtists}>
          <Icon size={40}>chevron_left</Icon>
        </BackButton>
        <HeaderContent icon="person" title={artist.name} />
      </Header>
      <VirtualList
        ref={paneRef}
        items={albums}
